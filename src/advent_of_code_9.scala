...
object ComputeString extends App {
  ...
  def decodeString(string: String): String = {
    ...
    if (opMarkerIndex.isEmpty) {
      ...
      val marker = string
        .substring(indexOfMarker._1, indexOfMarker._2)
        .split("x")
        .map(_.toInt)
      string.take(indexOfMarker._1 - 1) + string
        .drop(indexOfMarker._2 + 1)
        .take(marker(0)) * marker(1) + decodeString(
          string.substring((indexOfMarker._2 + 1 + marker(0))))
    }
  }
